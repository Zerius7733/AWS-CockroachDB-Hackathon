...
`agent/memory-store.js` uses CockroachDB whenever `DATABASE_URL` exists. Without it, the app uses `data/memory.json` only for local UI development. A hackathon deployment must set `DATABASE_URL`; local demo mode does not satisfy the persistent-memory requirement.
