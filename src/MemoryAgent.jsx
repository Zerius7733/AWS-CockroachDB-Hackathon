...
function SetupStatus({ status }) {
  const items = [
    { label: 'OpenAI agent', ready: status.openaiConfigured },
    { label: 'CockroachDB memory', ready: status.cockroachConfigured },
    { label: 'Cloud MCP', ready: status.mcpConfigured },
  ]
  return <div className="agent-status" aria-label="Agent connection status">
    {items.map((item) => <span className={item.ready ? 'ready' : ''} key={item.label}><i />{item.label}<small>{item.ready ? 'ready' : 'setup needed'}</small></span>)}
  </div>
}
...
export default function MemoryAgent({ onError, onNotify }) {
  ...
  const [status, setStatus] = useState({ openaiConfigured: false, cockroachConfigured: false, mcpConfigured: false, storage: 'local-demo' })
